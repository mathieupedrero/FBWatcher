...
public class FBWatcherConfiguration {
	...
	public synchronized void addTokenToProfile(String token,
			LocalDateTime expiracy, String profileId) {
	...
	public synchronized void addJob(String id, String eventFilter,
			boolean shouldMail, boolean shouldSms, boolean shouldAttend,
			String profileId) {
		Job job = new Job(id);
		job.setPageId(id);
	...
	public synchronized Set<Profile> retrieveProfiles() {
		return (Set<Profile>) SerializationUtils.deserialize(SerializationUtils
				.serialize(configuration.getProfiles()));
	}

	public synchronized Profile retrieveProfile(String profileId) {
		Optional<Profile> profile = configuration.getProfiles().stream()
				.filter(p -> Objects.equals(p.getId(), profileId)).findFirst();
		if (profile.isPresent()) {
			return (Profile) SerializationUtils.deserialize(SerializationUtils
					.serialize(profile.get()));
		}
		return null;
	}
	...
	public synchronized Set<Job> retrieveJobs() {
		return (Set<Job>) SerializationUtils.deserialize(SerializationUtils
				.serialize(configuration.getJobs()));
	}

	public synchronized void defineShouldMail(String jobId, boolean shouldMail) {
		configuration.getJobs().stream()
				.filter(j -> Objects.equals(j.getId(), jobId)).findFirst()
				.ifPresent(j -> j.setShouldMail(shouldMail));
		XmlFileUtils.writeToFile(configuration, fullConfigFilePath);
	}

	public synchronized void defineShouldAttend(String jobId,
			boolean shouldAttend) {
		configuration.getJobs().stream()
				.filter(j -> Objects.equals(j.getId(), jobId)).findFirst()
